...
export default function AnalyticsPage() {
  const [averageRatings, setAverageRatings] = useState<{ formTitle: string; averageRating: number }[]>([]);
  const [submissionCounts, setSubmissionCounts] = useState<{ formTitle: string; submissionCount: number }[]>([]);
  ...
  useEffect(() => {
    ...
    const sampleAverageRatings = [
      { formTitle: 'Form A', averageRating: 4.2 },
      { formTitle: 'Form B', averageRating: 3.8 },
      { formTitle: 'Form C', averageRating: 3.6 },
    ];

    // Sample data for Pie Chart
    const sampleSubmissionCounts = [
      { formTitle: 'Form A', submissionCount: 120 },
      { formTitle: 'Form B', submissionCount: 80 },
      { formTitle: 'Form C', submissionCount: 100 },
    ];

    // Sample data for Ratings Line Chart
    const sampleRatingsOverTime = [
      {
        formTitle: 'Form A (Growing)',
        averageRatings: [
          { date: '2023-01-01', averageRating: 0.5 },
          { date: '2023-02-01', averageRating: 2.4 },
          { date: '2023-03-01', averageRating: 3.2 },
          { date: '2023-04-01', averageRating: 4.1 },
          { date: '2023-05-01', averageRating: 5.0 },
        ],
      },
      {
        formTitle: 'Form B (Declining)',
        averageRatings: [
          { date: '2023-01-01', averageRating: 4.5 },
          { date: '2023-02-01', averageRating: 4.1 },
          { date: '2023-03-01', averageRating: 3.5 },
          { date: '2023-04-01', averageRating: 2.8 },
          { date: '2023-05-01', averageRating: 0.5 },
        ],
      },
      {
        formTitle: 'Form C (Stable)',
        averageRatings: [
          { date: '2023-01-01', averageRating: 3.4  },
          { date: '2023-02-01', averageRating: 3.4  },
          { date: '2023-03-01', averageRating: 3.4 },
          { date: '2023-04-01', averageRating:  3.4  },
          { date: '2023-05-01', averageRating: 3.4 },
        ],
      },
    ];
    ...
    // Simulate a delay
    setTimeout(() => {
      setAverageRatings(sampleAverageRatings);
      setSubmissionCounts(sampleSubmissionCounts);
      setRatingsOverTime(sampleRatingsOverTime);
      setLoading(false);
    }, 1000);
  }, []);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-gray-900 to-black text-white">
        <div className="text-lg animate-pulse mb-6">Loading data...</div>
