...
//       repeat: Infinity,
//       repeatType: 'loop', // Ensure this matches the allowed literal types
//       ease: 'linear',
//       delay: Math.random() * 1,
//     },
//   },
  ...
  useEffect(() => {
    const fetchAndProcessResponses = async () => {
      ...
      try {
        ...
        if (!geminiResponse.ok) throw new Error('Failed to process data with Gemini API');
  ...
  if (error) {
    return (
      <div className="text-center py-20 text-red-500 font-medium bg-gray-900 min-h-screen">
        ...
        <Link
          href={formId ? `/forms/${formId}/ai-analysis` : '/dashboard'}
  ...
  if (painPoints.length === 0) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-[#00b09b] to-[#96c93d] text-white text-2xl font-semibold">
        ...
        <Link
          href={formId ? `/forms/${formId}/ai-analysis` : '/dashboard'}
          className="ml-6 text-teal-400 hover:underline"
        >
          ← Back
        </Link>
      </div>
